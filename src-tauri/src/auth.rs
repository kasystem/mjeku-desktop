...
pub fn is_configured(db: &Db) -> anyhow::Result<bool> {
    ...
    Ok(clinic_id.as_deref().unwrap_or("").trim().len() > 0
...
pub fn setup_v2(
    ...
    let cashier_password = cashier_password
    ...
    let cashier_salt = cashier_password
        .as_deref()
...
pub fn session_get(db: &Db) -> anyhow::Result<SessionKind> {
    ...
    if raw.is_empty() {
        // Legacy fallback.
        if db
            .setting_get(KEY_USER_LOGGED_IN)?
            .unwrap_or_default()
            .trim()
            == "1"
        {
