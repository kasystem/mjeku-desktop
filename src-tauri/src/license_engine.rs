...
impl LicenseEngine {
    ...
    pub async fn check_now(&self) -> anyhow::Result<()> {
        ...
        let now = now_iso();
        if Self::detect_clock_tamper(self.db.as_ref(), &now)? {
            append_license_error(
                self.db.as_ref(),
                "license_check",
                "clock tampering detected: local device time moved backwards",
            );
            let st = LicensePublicState {
                ok: false,
                status: "clock_tampered".to_string(),
                active_until: self.db.setting_get(KEY_LICENSE_ACTIVE_UNTIL)?,
                last_checked_at: self.db.setting_get(KEY_LICENSE_LAST_CHECKED_AT)?,
            };
        ...
        let clinic_address = self.db.setting_get(KEY_CLINIC_ADDRESS)?.unwrap_or_default();
        ...
        let clinic_business_no = self
            ...
            .unwrap_or_default();
        let clinic_website = self.db.setting_get(KEY_CLINIC_WEBSITE)?.unwrap_or_default();
      ...
      .await;
        ...
        if !status.is_success() {
            append_license_error(
                self.db.as_ref(),
                "license_check",
                &format!("license check failed: {status} {body}"),
            );
        ...
        let mut rows: Vec<ClinicRegistryRow> =
            serde_json::from_str(&body).context("decode clinic_registry json")?;
        if rows.is_empty() {
            ...
            self.db.setting_set(
                KEY_LICENSE_LAST_PUBLIC_IP,
                &public_ip.clone().unwrap_or_default(),
            )?;
        ...
        let heartbeat = with_supabase_auth(
            self.client.patch(format!(
                "{url}?clinic_id=eq.{}",
                urlencoding::encode(clinic_id.trim())
            )),
            &api_key,
        )
        ...
        self.db
            .setting_set(KEY_LICENSE_ALLOWED_IP_LIST, &allowed_ip_list)?;
        self.db.setting_set(
            KEY_LICENSE_LAST_PUBLIC_IP,
            &public_ip.clone().unwrap_or_default(),
        )?;
        ...
        if ok && enforce_ip {
            let ip_to_check = public_ip.clone().or_else(|| {
                self.db
                    .setting_get(KEY_LICENSE_LAST_PUBLIC_IP)
                    .ok()
                    .flatten()
            });
    ...
    fn compute_offline_state(db: &Db) -> anyhow::Result<LicensePublicState> {
        let now = now_iso();
        if Self::detect_clock_tamper(db, &now)? {
            return Ok(LicensePublicState {
                ok: false,
                status: "clock_tampered".to_string(),
                active_until: db.setting_get(KEY_LICENSE_ACTIVE_UNTIL)?,
                last_checked_at: db.setting_get(KEY_LICENSE_LAST_CHECKED_AT)?,
            });
        }
        ...
        let clinic_id = db.setting_get(KEY_CLINIC_ID)?.unwrap_or_default();
        if clinic_id.trim().is_empty() {
            return Ok(LicensePublicState {
                ok: true,
                status: "unconfigured".to_string(),
        ...
        let disabled = db
            .setting_get(KEY_LICENSE_DISABLED)?
            .unwrap_or_default()
            .trim()
            == "1";
        let approved = db
            .setting_get(KEY_LICENSE_APPROVED)?
            .unwrap_or_default()
            .trim()
            == "1";
        let enforce_ip = db
            .setting_get(KEY_LICENSE_ENFORCE_IP)?
            .unwrap_or_default()
            .trim()
            == "1";
        let allowed_ip_list = db
            .setting_get(KEY_LICENSE_ALLOWED_IP_LIST)?
            .unwrap_or_default();
        let last_public_ip = db
            .setting_get(KEY_LICENSE_LAST_PUBLIC_IP)?
            .unwrap_or_default();
        ...
        if enforce_ip
            && !allowed_ip_list.trim().is_empty()
            && !ip_allowed(&last_public_ip, &allowed_ip_list)
        {
            return Ok(LicensePublicState {
                ok: false,
        ...
        if let Some(u) = active_until.as_deref() {
            if let (Ok(until), Ok(now_dt)) = (parse_rfc3339_to_utc(u), parse_rfc3339_to_utc(&now)) {
                if now_dt > until {
                    return Ok(LicensePublicState {
                        ok: false,
        ...
        if let Some(ts) = last_checked_at.as_deref() {
            if let (Ok(last), Ok(now_dt)) = (parse_rfc3339_to_utc(ts), parse_rfc3339_to_utc(&now)) {
                let days = (now_dt - last).num_days();
                if days <= GRACE_DAYS {
                    return Ok(LicensePublicState {
                        ok: true,
    ...
    fn detect_clock_tamper(db: &Db, now_iso_str: &str) -> anyhow::Result<bool> {
        let prev = db
            .setting_get(KEY_LICENSE_LAST_SEEN_DEVICE_TIME)?
            .unwrap_or_default();
        let prev = prev.trim().to_string();
        if !prev.is_empty() {
            if let (Ok(prev_dt), Ok(now_dt)) = (
                parse_rfc3339_to_utc(&prev),
                parse_rfc3339_to_utc(now_iso_str),
            ) {
                let delta = prev_dt - now_dt;
                if delta.num_seconds() > CLOCK_BACKWARD_TOLERANCE_SECS {
                    return Ok(true);
                }
            }
        }
        db.setting_set(KEY_LICENSE_LAST_SEEN_DEVICE_TIME, now_iso_str)?;
        Ok(false)
    }
}
